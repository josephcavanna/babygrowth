import 'dart:io';
import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
import 'package:flutter_icons/flutter_icons.dart';

class ProfilePhoto {

  // Widget that displays the profile photo of the baby
  Positioned profilePhotoWidget(
      {@required BuildContext context, @required String babyName, @required Color color, @required File babyImage, @required ImageProvider<
          Object> backgroundImage}) {
    return Positioned(
      top: MediaQuery
          .of(context)
          .orientation == Orientation.portrait
          ? 117.0
          : MediaQuery
          .of(context)
          .size
          .aspectRatio >= 16 / 9
          ? 40
          : 80,
      left: MediaQuery
          .of(context)
          .orientation == Orientation.portrait
          ? (MediaQuery
          .of(context)
          .size
          .width / 2 - 80)
          : MediaQuery
          .of(context)
          .size
          .aspectRatio >= 16 / 9
          ? (MediaQuery
          .of(context)
          .size
          .width / 2 - 40)
          : (MediaQuery
          .of(context)
          .size
          .width / 2 - 80),
      child: Container(
        child: Hero(
          tag: babyName,
          child: Material(
            type: MaterialType.circle,
            color: Colors.white,
            elevation: 4,
            child: Container(
              child: CircleAvatar(
                backgroundColor: Colors.grey[300],
                backgroundImage: babyImage != null ? backgroundImage : Icon(FontAwesome5Solid.baby),
                child: babyImage == null
                    ? Icon(
                  FontAwesome5Solid.baby,
                  size: 50,
                  color: color,
                )
                    : null,
              ),
            ),
          ),
        ),
        height: avatarRadius(context),
        width: avatarRadius(context),
      ),
    );
  }

  // This determines the radius of the profile photo
  double avatarRadius(BuildContext context) {
    return MediaQuery
        .of(context)
        .orientation == Orientation.portrait
        ? 160.0
        : MediaQuery
        .of(context)
        .size
        .aspectRatio >= 16 / 9
        ? 80
        : 160;
  }

}